...
def pipeline(args):
    ...
    set_seed(args.seed)
    save_path = f'results/{args.pipeline_name}/{args.task.env_name}_random_classifier/{timestamp}/'
    ...
    if args.mode == "train":
        # folder_path = f'results/{args.pipeline_name}/pretrained/'
        # classifier_files = [f for f in os.listdir(folder_path) if f.startswith('critic') and f.endswith('.pt') and re.search(r'\d+', f)]
        # critic_ckpt = torch.load(f"{folder_path}{classifier_files[0]}", map_location=args.device)
        ...
        for batch in loop_dataloader(dataloader):
            ...
            if n_gradient_step >=args.retrain_classifier_step and not retrain_classifier:
                for param in critic.parameters():
                    param.requires_grad = True
                critic.train()
                retrain_classifier = True

            if n_gradient_step >args.retrain_classifier_step and retrain_classifier:
