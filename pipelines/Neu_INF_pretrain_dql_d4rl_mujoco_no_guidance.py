...
def record_result(guidance_name, step, score, std, pipeline_name, env_name):
    ...
    log_csv = f"log_results_{pipeline_name}_{env_name}_{timestamp}_no_guidance.csv"
...
def save_results_to_csv(results_dict, pipeline_name, env_name):
    ...
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    output_csv = f"guidance_scores_{pipeline_name}_{env_name}_{timestamp}_no_guidance.csv"
...
def get_guidance_dirs(base_path):
    return [os.path.join(base_path, d) for d in os.listdir(base_path)
            if os.path.isdir(os.path.join(base_path, d)) and "guidance_0" in d]
...
def load_ckpt_and_inference(guidance_name, step, actor, critic, critic_target,
    ...
    critic_state = torch.load(critic_ckpt)
...
def pipeline(args):
    ...
    save_path = f"results/{args.pipeline_name}/{args.task.env_name}_no_guidance/"
