...
def get_ckpts_with_fallback(guidance_dir, fallback_dir):
    ...
    critic_ckpts = {
        extract_step(f): os.path.join(fallback_dir, f)
...
def record_result(guidance_name, step, score, std, pipeline_name, env_name, diffusion_ckpt, critic_ckpt, seed):
    results_dict[step][guidance_name] = (diffusion_ckpt, critic_ckpt, score, std)
    
    log_csv = f"log_results_{pipeline_name}_{env_name}_{timestamp}_{seed}_D_baseline.csv"
...
def save_results_to_csv(results_dict, pipeline_name, env_name,seed):
    ...
    output_csv = f"guidance_scores_{pipeline_name}_{env_name}_{timestamp}_seed{seed}_D_baseline.csv"
    all_guidances = sorted({g for step in results_dict for g in results_dict[step].keys()})
    
    data = {"step": steps}
    for g in all_guidances:
        data[f"{g}_mean"] = [results_dict[step].get(g, (None, None, None, None))[2] for step in steps]
        data[f"{g}_variance"] = [results_dict[step].get(g, (None, None, None, None))[3]**2 if results_dict[step].get(g) else None for step in steps]
...
def get_guidance_dirs(base_path):
    return [os.path.join(base_path, d) for d in os.listdir(base_path)
            if os.path.isdir(os.path.join(base_path, d)) and "GFDT" not in d and "pretrained" not in d]

def load_ckpt_and_inference(guidance_name, step, actor, critic, critic_target,
                             diffusion_ckpt, critic_ckpt, args, dataset, env_eval,seed):
    actor.load(diffusion_ckpt)
    
    if critic_ckpt:  # 检查 critic_ckpt 是否为空
    ...
    env = gym.make(args.task.env_name)  # 创建单独的环境用于计算标准化分数
    ...
    record_result(guidance_name, step, mean_score, mean_std, args.pipeline_name, args.task.env_name, diffusion_ckpt, critic_ckpt,seed)

@hydra.main(config_path="../configs/edp/antmaze", config_name="antmaze", version_base=None)
def pipeline(args):
    set_seed(args.seed)

    save_path = f'results/{args.pipeline_name}_{args.seed}/{args.task.env_name}/'
    fallback_dir = f'results/{args.pipeline_name}_{args.seed}/{args.task.env_name}/pretrained'
    ...
    save_path1 = f'results_0/{args.pipeline_name}/{args.task.env_name}/'
    save_path2 = f'seed123_dql/{args.pipeline_name}_123/{args.task.env_name}/'
    g_dir1,g_dir2 =sorted(get_guidance_dirs(save_path1))[0],sorted(get_guidance_dirs(save_path2))[0]
    guidance_name1 = os.path.basename(g_dir1)
    guidance_name2 = os.path.basename(g_dir2)
    print(f"🔍 Evaluating: {guidance_name1}")
    step_ckpt_pairs = sorted(get_ckpts_with_fallback(g_dir1,g_dir2),reverse=True)
    if not step_ckpt_pairs:
        print(f"⚠️ No ckpt found in {guidance_name1}")

    
    # Sort by step in descending order (latest first)
    # step_ckpt_pairs = sorted(step_ckpt_pairs, key=lambda x: x[0])
    
    for step, diff_ckpt, critic_ckpt in step_ckpt_pairs:
        try:
            load_ckpt_and_inference(guidance_name1, step, actor, critic, critic_target,
                                diff_ckpt, critic_ckpt, args, dataset, env_eval,args.seed)
        except:
            pass
    save_results_to_csv(results_dict, args.pipeline_name, args.task.env_name,args.seed)
