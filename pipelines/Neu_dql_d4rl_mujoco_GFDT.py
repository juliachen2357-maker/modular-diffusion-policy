...
def pipeline(args):
    args.save_interval=400
    args.gradient_steps=1000000
    args.device = "cuda:0" if torch.cuda.is_available() else "cpu"
    set_seed(args.seed)
    if not torch.cuda.is_available():
        args.device='cpu'
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    args.experiment_name=f'{args.seed}_GFDT'
    experiment_tag = f"{args.experiment_name}_{timestamp}"
    save_path = f'results/{args.pipeline_name}/{args.task.env_name}_pretrain/{experiment_tag}/'
    save_path = os.path.join(save_path, timestamp) + '/'
    print(f'{args.pipeline_name}"AND"{args.task.env_name}')
